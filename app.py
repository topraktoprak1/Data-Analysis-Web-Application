...
@login_required
def get_admin():
    """Get admin profile info (same as user for now)"""
    return jsonify({
        'admin': {
            'username': session.get('user'),
            'name': session.get('name'),
...
def get_filter_options():
    ...
    try:
        ...
        for record in data:
            ...
            # Check each currently applied filter category
            # Within each category, use OR logic (match ANY selected value)
            # Across categories, use AND logic (must match at least one value from EACH category)
            for filter_key, field_names in filter_mapping.items():
                ...
                if filter_values and len(filter_values) > 0:
                    ...
                    # Check if record matches ANY of the selected values (OR logic within category)
                    if record_value:
                        try:
                            ...
                            # If record does NOT match any of the selected values, exclude it
...
def get_mh_table_data():
    ...
    try:
        ...
        # Process records - aggregate by unique combination of (person, discipline, company, projectsGroup)
        person_data = {}
        print(f"[MH TABLE] Starting aggregation with {len(filtered_data)} filtered records")
        ...
        for record in filtered_data:
            ...
            # Get other attributes to create unique key
            ...
            # Create unique key combining all relevant attributes
            unique_key = f"{name}|{discipline}|{company}|{projectsGroup}"
            ...
            if year and rec_year and str(rec_year) != year:
                print(f"[MH TABLE] Skipping record - year mismatch: rec_year={rec_year}, filter_year={year}")
                continue
            
            # Initialize person entry if not exists (unique by combination)
            if unique_key not in person_data:
                person_data[unique_key] = {
                    'nameSurname': name,
            ...
            if rec_month and rec_year:
                ...
                if not year or str(rec_year) == year:
                    # If no year filter is set, include year in the key for frontend grouping
                    if year:
                        # Year filter is set, use simple month key
                        month_key = str(rec_month).zfill(2)
                    else:
                        # No year filter, include year-month for grouping by year
                        month_key = f"{rec_year}-{str(rec_month).zfill(2)}"
                    
                    # Apply month filter if specified
                    if not month or str(rec_month).zfill(2) == month:
                        person_data[unique_key]['monthlyMH'][month_key] = \
                            person_data[unique_key]['monthlyMH'].get(month_key, 0) + total_mh
                        person_data[unique_key]['totalMH'] += total_mh
            else:
        ...
        print(f"[MH TABLE] Created {len(person_data)} unique keys in person_data")
        print(f"[MH TABLE] Returning {len(result)} aggregated person records")
        print(f"[MH TABLE] Sample names: {[r['nameSurname'] for r in result[:5]]}")
        print(f"[MH TABLE] Sample unique keys: {list(person_data.keys())[:3]}")
