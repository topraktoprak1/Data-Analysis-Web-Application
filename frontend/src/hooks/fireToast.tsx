...
const fireToast = async () => {
  ...
  try {
    dataJSON = await api.getRecords();
  } catch (err) {
    console.warn('fireToast: failed to load records', err);
    return;
  }
  ...
  for (const alertSetting of JSON.parse(alertSettings)) {
    ...
    if (alertSetting.id === 'ALL') {
      for (const id of Object.keys(dataJSON)) {
        const v = dataJSON[id][para];
        const condition = alertSetting.criterion == '0' ? value <= -1 * v :
          alertSetting.criterion == '1' || alertSetting.criterion == '3' ? value >= v :
            alertSetting.criterion == '2' ? value <= v :
              value == v;
        const realValue = alertSetting.criterion == '0' ? v * -1 : v;
        if (condition) createToast(id, `${alertSetting.para} of ${id} ${alertSetting.criterion == 0 ? 'goes down by' : alertSetting.criterion == 1 ? 'goes up by' : alertSetting.criterion == 2 ? 'is smaller than' : alertSetting.criterion == 3 ? 'is greater than' : 'is equal to'} ${realValue}`, alertSetting.type);
      }
    } else {
      const id = alertSetting.id;
      const v = dataJSON[id] ? dataJSON[id][para] : undefined;
